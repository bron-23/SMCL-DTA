...
for EPOCH in 1317 1323 1344; do
  CKPT="$(find /home/lww/learn_project/mydta/checkpoints -name "epoch-${EPOCH}*.pt" | head -n 1)"

  if [[ -z "$CKPT" ]]; then
    echo "[ERROR] checkpoint for epoch-${EPOCH} not found"
    exit 1
  fi

  OUT_DIR="${OUT_ROOT}/finetune_epoch${EPOCH}_full"
  LOG="${OUT_ROOT}/finetune_epoch${EPOCH}_full.log"

  mkdir -p "$OUT_DIR"

  echo "================================================================================"
  echo "[START] Fine-tuning epoch-${EPOCH}"
  echo "[CKPT] $CKPT"
  echo "[OUT]  $OUT_DIR"
  echo "[LOG]  $LOG"
  echo "================================================================================"
  ...
  echo "[DONE] Fine-tuning epoch-${EPOCH}"
  echo "[BEST]"
...
echo "================================================================================"
echo "[ALL DONE] Remaining checkpoints fine-tuned."
echo "================================================================================"
